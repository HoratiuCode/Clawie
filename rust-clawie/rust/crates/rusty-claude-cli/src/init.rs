...
const GITIGNORE_COMMENT: &str = "# Clawie local artifacts";
...
impl InitReport {
    ...
    pub(crate) fn render(&self) -> String {
        ...
        lines.push("  Launch           ./clawie from the repo root".to_string());
        lines.join("\n")
    }
}
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Clawie when working with code in this repository.".to_string(),
        String::new(),
    ];
