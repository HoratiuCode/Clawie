...
fn main() {
    ...
    let mut f = BufWriter::new(File::create(&dest_path).expect("Failed to create slash_commands.rs"));

    // Read commands.json
    let commands_json = std::fs::read_to_string("commands.json").expect("Failed to read commands.json");
    let commands: serde_json::Value = serde_json::from_str(&commands_json).expect("Failed to parse commands.json");
    let commands_array = commands.as_array().expect("commands.json must be a JSON array");
    ...
    for cmd in commands_array {
        ...
        let aliases_strs: Vec<String> = aliases_val.iter().map(|v| format!("\"{}\"", v.as_str().unwrap())).collect();
    ...
    writeln!(f, "fn slash_command_category(name: &str) -> &'static str {{").unwrap();
    ...
    for cmd in commands_array {
        ...
        categories_map.entry(category.to_string()).or_insert_with(Vec::new).push(name.to_string());
    }
